...
class AuthHelper
{
	...
	AuthHelper(RapidExecutor executor, AuthCallback callback, RapidLogger logger)
	{
		mExecutor = executor;
		mCallback = callback;
		mLogger = logger;
	...
	RapidFuture deauthorize(ConnectionState connectionState)
	{
		...
		RapidFuture deauthFuture = new RapidFuture(mExecutor);
	...
	{
		mAuthFuture = new RapidFuture(mExecutor);
