...
class CollectionProvider {
	...
	CollectionProvider(RapidConnection connection, JsonConverterProvider jsonConverter, RapidExecutor executor, SubscriptionDiskCache subscriptionDiskCache, RapidLogger debugLogger, CacheProvider cacheProvider) {
	...
	<T> RapidCollectionReference<T> provideCollection(String collectionName, Class<T> itemClass) {
		if(!mCollectionConnections.containsKey(collectionName))
			mCollectionConnections.put(collectionName, new WebSocketCollectionConnection<>(mConnection, mJsonConverter, collectionName, itemClass, mSubscriptionDiskCache, mDebugLogger, mExecutor, mCacheProvider));
	...
	RapidCollectionReference<Map<String, Object>> provideCollection(String collectionName) {
		if(!mCollectionConnections.containsKey(collectionName))
			mCollectionConnections.put(collectionName, new WebSocketCollectionConnection<>(mConnection, mJsonConverter, collectionName, Map.class, mSubscriptionDiskCache, mDebugLogger, mExecutor, mCacheProvider));
