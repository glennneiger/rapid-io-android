...
class WebSocketCollectionConnection<T> implements CollectionConnection<T> {
	...
	WebSocketCollectionConnection(RapidConnection connection, JsonConverterProvider jsonConverter, String collectionName, Class<T> type, SubscriptionDiskCache subscriptionDiskCache, RapidLogger logger, RapidExecutor executor, CacheProvider cacheProvider) {
		mCollectionName = collectionName;
		mConnection = connection;
		mJsonConverter = jsonConverter;
		mType = type;
		mExecutor = executor;
		mSubscriptionMemoryCache = new SubscriptionMemoryCache<>(cacheProvider);
